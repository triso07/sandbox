...
// !!!!!!!!!!!!!!!!!!!! IMPORTS !!!!!!!!!!!!!!!!!!!! //
import { User, Budget, BudgetItem } from 'classes'; 
...
function addDataUser(name, types, active, cb) {
	users.set(name, new User(name, new Budget(types), active));
	if (cb) { cb(users.get(name)); }
}
...
function addDataItem(itemType, domValues, cbDisplayChanges) {
	...
	currUser.budget[itemType].items.set(domValues.id, new BudgetItem(domValues.desc, domValues.val));
	// calculate totals and percentages
	doCalculataions(currUser.budget, itemType);
	// return to UI with data
	if (cbDisplayChanges) { cbDisplayChanges(currUser.budget); }
}
